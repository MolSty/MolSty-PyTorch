...
#     seen = set()
#     data = [d for d in data if Chem.MolFromSmiles(d[0]) is not None \
...
#                 and ent[1] not in decoy_smis_gen: # Check conf and not a decoy for another ligand
...
# =============================================================================
    ...
    with open(output_name, 'w') as outfile:
        for i, smi in enumerate(decoy_smis_gen):
            outfile.write(set_in_smis[i//num_dec_per_act] + ' ' + smi + '\n')
    print(dec_results)
    GM = np.mean(dec_results[7+1:7+1+4])
    print(f'GM: {GM:.4f}')

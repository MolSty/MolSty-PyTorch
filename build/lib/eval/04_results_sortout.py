...
target_lst = sorted([item.split('-')[2].upper() for item in target_lst])

fn_lst = os.listdir('./eval')
fn_lst = [item.lower() for item in fn_lst]
max_idx_cmpd = 10000
min_active_size = 10# int(args.min_active_size)
output_loc = './eval/results/'
dataset = 'dude'
if os.path.exists('./eval/results.csv'):
    df = pd.read_csv(
        './eval/results.csv', header=None, 
        names=['File name', 'Dataset', 'Orig num actives', 'Num actives', 
               'Num generated mols', 'Num unique gen mols',
               'AUC ROC - 1NN', 'AUC ROC - RF', 'DOE score',
               'LADS score', 'Doppelganger score mean', 'Doppelganger score max',
               'GM'])
    print(df.head())
    s = set([item.split('_')[0] for item in df['File name'].values])
    print(s)
else:
    s = set()
with open('./eval/results.csv', 'a') as csvfile:
    writer = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    for target in target_lst:
        ...
        run(output_name, output_name, topk=N*10)

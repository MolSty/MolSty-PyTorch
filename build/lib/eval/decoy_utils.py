...
def parallel_func(inputs, func):
    results = Parallel(n_jobs=20,backend='loky')(delayed(func)(x) for x in tqdm(inputs))
    return results


# CALC FUNCTIONS
def calc_props_dude(mol):
    try:
...
def calc_props_dude_extended(mol):
    try:
        # mol = Chem.MolFromSmiles(smiles)
        if type(mol) is str:
            mol = Chem.MolFromSmiles(mol)
...
# =============================================================================
    probs_lst = parallel_func(dataset, calc_props_all)
    for i, props in enumerate(probs_lst):
        if props is not None:
            results.append(props)
        if verbose and i % 1000 == 0:
            print("\rProcessed smiles: " + str(i), end='')
...
#     print("\nDone calculating properties")
# =============================================================================
    probs_lst = parallel_func(dataset, calc_props_dude)
    for i, props in enumerate(probs_lst):
        if props is not None:
            results.append(props[1])
        if verbose and i % 1000 == 0:
...
def lads_score_v2(actives, decoys):
    ...
    decoys_fps = [AllChem.GetMorganFingerprint(m,3,useFeatures=True) for m in decoys] # Roughly FCFP_6
...
def dg_score(actives, decoys):
    # Similar to DEKOIS
    # Lower is better (less like actives), higher is worse (more like actives)
    active_fps = [AllChem.GetMorganFingerprintAsBitVect(mol,3,useFeatures=True) for mol in actives] # Roughly FCFP_6
    decoys_fps = [AllChem.GetMorganFingerprintAsBitVect(mol,3,useFeatures=True) for mol in decoys] # Roughly FCFP_6
...
def calc_AUC(active_props, decoy_props, idxs, random_state=42, n_jobs=-1):
    # Scale properties based on active props
    active_min_all = []
    active_max_all = []
    active_scale_all = []
    ...
    # Labels
    y = np.array([1]*len(active_props) + [0]*len(decoy_props))
    print(len(active_props), len(decoy_props))
    # Construct x-val folds
    # strat_kfold = StratifiedKFold(n_splits=10, shuffle=False, random_state=42)
    ...
    perf_1nn = []
    perf_rf = []
    idxs_to_split = np.array(list(set(idxs)))
    for tr_idx, te_idx in kf.split(idxs_to_split):
        # Train and test model
        tr_idx, te_idx = idxs_to_split[tr_idx], idxs_to_split[te_idx]
        train_idx = [i for i, item in enumerate(idxs) if item in tr_idx]
        test_idx = [i for i, item in enumerate(idxs) if item in te_idx]
        train_idx = train_idx + list(np.array(train_idx)+len(idxs))
        test_idx = test_idx + list(np.array(test_idx)+len(idxs))
        print(train_idx)
        print(test_idx)
        X_train, X_test = X[train_idx], X[test_idx]
        y_train, y_test = y[train_idx], y[test_idx]

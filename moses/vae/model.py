...
class VAE(nn.Module):
    def __init__(self, vocab, config):
        ...
        self.content_predictor = nn.Linear(128, 8)
    ...
    def forward_encoder(self, x, return_seq=False):
        ...
        output, h = self.encoder_rnn(x, None)
        if return_seq:
            '''需要sort后再pack'''
            def unpack_gru(output):
                bs = output.batch_sizes.shape[0]
                output_seq = [[] for i in range(bs)]
                batch_sizes = output.batch_sizes.data.cpu().numpy()
                idx = 0
                for i in range(output.batch_sizes.max().item()):
                    for j in range(bs):
                        if batch_sizes[j] > 0:
                            output_seq[j].append(output.data[idx].data.cpu().numpy())
                            idx += 1
                            batch_sizes[j] -= 1
                output_seq = [np.stack(item) for item in output_seq]
                return output_seq
            output_seq = unpack_gru(output)
    ...
    def decode(self, z, max_len=100, temp=1.0):
        """Generating n_batch samples in eval mode (`z` could be
        not on same device)
    ...
    def encode(self, x):
        ...
        z = []
        with torch.no_grad():
            for i in range(0, math.ceil(len(x)/bs)):
                tmp_z, _ = self.forward_encoder(x[i*bs:min([(i+1)*bs, len(x)])])
                z.append(tmp_z)
        return torch.cat(z, 0).data.cpu().numpy()